%% Compress and decode the reduced EBD frames

% Path to save decoded reduced EBD still image
DecForemanStillPath = './DecFrame/Still/';
if ~exist(DecForemanStillPath, 'dir')
    mkdir(DecForemanStillPath);
else
    rmdir(DecForemanStillPath,"s");
    mkdir(DecForemanStillPath);
end
% Path to save decoded reduced EBD video frame
DecFrameVideoPath = './DecFrame/Video/';
if ~exist(DecFrameVideoPath, 'dir')
    mkdir(DecFrameVideoPath);
else
    rmdir(DecFrameVideoPath,"s");
    mkdir(DecFrameVideoPath);
end
% read all original frames in the cell variable 'OriFrame' and all reduced
% EBD frames in the cell variable 'EBDFrame'
...
for i = 1:videoLength
    ...
    EBDFrame{i}= bitshift(OriFrame{i},-1); % reduce bit depth of original frame by 1 using 'bitshift' function
end
EBDFrameYUV = cellfun(@ictRGB2YCbCr, EBDFrame, 'Uniform', false);
LenaSmall = bitshift(double(imread('lena_small.tif')),-1);
LenaSmallYUV = ictRGB2YCbCr(LenaSmall);
scales = [0.07 0.1 0.2 0.4 0.8 1.0 1.5 2.0 3.0 4.0 4.5];

%% Still image compression and decode
BPPStill  = zeros(1,8);
BPPVideo  = zeros(1,8);
range = -1000:4000;
for scaleIdx = 1 : numel(scales)
    qScale1   = scales(scaleIdx);
    EBDStillFramePath = [DecForemanStillPath,'QP',num2str(qScale1),'/'];
    if ~exist(EBDStillFramePath, 'dir')
        mkdir(EBDStillFramePath);
    else
        rmdir(EBDStillFramePath,"s");
        mkdir(EBDStillFramePath);
    end
    bpp = 0;
    psnr = 0;
    LenaSmall  = IntraEncode(LenaSmallYUV, qScale1);
    % use pmf of lena_small to build and train huffman table  
    pmf = hist(LenaSmall(:), range);
    pmf = pmf ./ sum(pmf);
    [BinaryTree, HuffCode, BinCode, Codelengths] = buildHuffman(pmf);
    for i = 1 : videoLength
        % k and k_small: run-level encoded results in 1D vector [Y, Cb, Cr]
        ForemanYUV = EBDFrameYUV{i};
        k          = IntraEncode(ForemanYUV, qScale1);
        % use trained table to encode k to get the bytestream
        LowerBand = -1000;
        bytestream = enc_huffman_new(k-LowerBand+1, BinCode, Codelengths);
        % calculate bit rate
        bpp = bpp + (numel(bytestream)*8) / (numel(EBDFrame{i})./3);
        % decoding
        k_rec = double(reshape(dec_huffman_new(bytestream, BinaryTree,...
            max(size(k(:)))), size(k))) - 1 + LowerBand;
        % image reconstruction
        I_rec = IntraDecode(k_rec, size(EBDFrame{i}), qScale1);
        I_rec = ictYCbCr2RGB(I_rec);
        imwrite(uint8(I_rec),[EBDStillFramePath,'Frame_',num2str(i),'.bmp']);
        
    end
    BPPStill(scaleIdx) = bpp ./ videoLength;
    fprintf('Still QP: %.1f bit-rate: %.2f bits/pixel \n', ...
        qScale1, BPPStill(scaleIdx))
end
dlmwrite('BPPStill.txt',BPPStill,'precision','%.6f');

%% Video compression and decode
BPP  = zeros(1,videoLength);
for scaleIdx = 1 : numel(scales)
    qScale2   = scales(scaleIdx);
    EBDVideoFramePath = [DecFrameVideoPath,'QP',num2str(qScale2),'/'];
    if ~exist(EBDVideoFramePath, 'dir')
        mkdir(EBDVideoFramePath);
    else
        rmdir(EBDVideoFramePath,"s");
        mkdir(EBDVideoFramePath);
    end
    RecFrame = cell(videoLength, 1);
    FinalRecFrame = cell(videoLength, 1);
    FinalRecFrameRGB = cell(videoLength, 1);
    % encode 1st frame using still image compression
    Frame1stYUV = EBDFrameYUV{1};
    zeroRunLenaSmall = IntraEncode(LenaSmallYUV, qScale2);
    zeroRunFrame1st  = IntraEncode(Frame1stYUV, qScale2);
    RecFrame{1} = IntraDecode(zeroRunFrame1st, size(OriFrame{1}), qScale2);
    % use pmf of zeroRunLenaSmall to build and train huffman table
    pmfFrame1st = hist(zeroRunFrame1st(:),min(zeroRunFrame1st):max(zeroRunFrame1st));  % based on the actual range of encoded image
    pmfFrame1st = pmfFrame1st./sum(pmfFrame1st); 
    % Huffman Table
    [BinaryTreeFirstFrame, HuffCodeFirstFrame, BinCodeFirstFrame, CodelengthsFirstFrame] = buildHuffman(pmfFrame1st');
    % use trained table to encode zeroRunFirstFrame to get the bytestream
    LowerBoundFrame1st = min(zeroRunFrame1st);  % based on the actual range of encoded image
    bytestreamZeroRun = enc_huffman_new(zeroRunFrame1st-LowerBoundFrame1st+1, BinCodeFirstFrame, CodelengthsFirstFrame);
    % calculate bit rate
    bitrate(1) = (numel(bytestreamZeroRun)*8) / (numel(OriFrame{1})/3);
    % decoding
    decZeroRun = double(reshape(dec_huffman_new(bytestreamZeroRun, BinaryTreeFirstFrame,...
        max(size(zeroRunFrame1st(:)))), size(zeroRunFrame1st))) - 1 + LowerBoundFrame1st;
    % image reconstruction
    FinalRecFrame{1} = IntraDecode(decZeroRun, size(OriFrame{1}), qScale2); 
    FinalRecFrameRGB{1} = ictYCbCr2RGB(FinalRecFrame{1});
    imwrite(uint8(FinalRecFrameRGB{1}),[EBDVideoFramePath,'Frame_1.bmp']);
    % encode the rest of frames
    for i = 2 : videoLength
        RefFrameYUV = RecFrame{i-1};  % last reconst. frame
        CurrentFrameYUV = EBDFrameYUV{i};
        MV = SSD(RefFrameYUV(:,:,1), CurrentFrameYUV(:,:,1));  % only Y component
        Predict = SSD_rec(RefFrameYUV, MV);  % predicted current img, YCbCr
        Err = CurrentFrameYUV - Predict;  % prediction error
        zeroRun = IntraEncode(Err, qScale2);
        RecFrame{i} = IntraDecode(zeroRun, size(CurrentFrameYUV), qScale2) + Predict;  % YCbCr
        %% Part 2: build Huffman table, compute only once
        if i == 2
            % use pmf to build and train huffman table
            pmfMV = hist(MV(:),range);
            pmfMV = pmfMV./sum(pmfMV);
            pmfZeroRun = hist(zeroRun(:),range);
        ...
        % use trained table to encode & decode MV and residual image
        % encoding
        ...
        % calculate bit rate
        rateZeroRun = (numel(bytestreamZeroRun)*8) / (numel(CurrentFrameYUV)./3); 
        rateMV = (numel(bytestreamMV)*8) / (numel(CurrentFrameYUV)./3);
        ...
        % image reconstruction
        Predict2 = SSD_rec(FinalRecFrame{i-1}, decMV);
        FinalRecFrame{i} = IntraDecode(decZeroRun, size(CurrentFrameYUV), qScale2) + Predict2;
        FinalRecFrameRGB{i} = ictYCbCr2RGB(FinalRecFrame{i});
        imwrite(uint8(FinalRecFrameRGB{i}),[EBDVideoFramePath,'Frame_',num2str(i),'.bmp']);
        BPP(i) = rateZeroRun + rateMV;
        
    end
    
    BPPVideo(scaleIdx) = mean(BPP);
    fprintf('Video QP: %.1f bit-rate: %.2f bits/pixel \n', ...
        qScale2, BPPVideo(scaleIdx))
    
end
% Write bit rate after optimization to a '.txt' file so that we can read it and plot the final
% results
dlmwrite('BPPVideo.txt',BPPVideo,'precision','%.6f');

%% Split decoded reduced EBD frames to blocks

EBDFrame = cell(videoLength, 1);
VideoOrImg = ["Still Image","Video"];
for x = 1:2
    for scaleIdx = 1 : numel(scales)
        qScale   = scales(scaleIdx);
        switch VideoOrImg(x)
            case 'Still Image'
                % This is the folder path of the reduced EBD frames after
                % decoding and please do not change it.
                DecPath = './DecFrame/Video/';
                EBDFrameFolder = [DecPath,'QP',num2str(qScale),'/'];
                
                % This is the folder path to save the splited frame blocks.
                % You can choose a folder by yourself.
                EBDBlockFolder = './Block/Still/';
            case 'Video'
                % This is the folder path of the reduced EBD frames after
                % decoding and please do not change it. This folder will be
                % used by CNN prediction.
                DecPath = './DecFrame/Still/';
                EBDFrameFolder = [DecPath,'QP',num2str(qScale),'/'];
                
                % This is the folder path to save the splited frame blocks.
                % You can choose a folder by yourself. This folder will be
                % used by CNN prediction.
                EBDBlockFolder = './Block/Video/';
        end
        EBDBlockPath = [EBDBlockFolder,'QP',num2str(qScale),'/'];
        ...
        for i = 1:videoLength
            EBDFrame{i} = double(imread([EBDFrameFolder, 'Frame_',num2str(i),'.bmp']));
        end
        % Pad frames with the model 'replicate' so that we can split the
        % frame to 96x96x3 blocks.
