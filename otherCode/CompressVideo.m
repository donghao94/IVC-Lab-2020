% The folder path to save the encoded reduced EBD frames.
DecFramePath = '/Users/donghao/Desktop/DecForeman_Video/';
if ~exist(DecFramePath, 'dir')
    mkdir(DecFramePath);
else
    rmdir(DecFramePath,"s");
    mkdir(DecFramePath);
end

% read all original frames in the cell variable 'OriFrame'
...
for i = 1:videoLength
    ...
    % Reduce the bit depth by one using bitshift function.
    EBDFrame{i}= bitshift(OriFrame{i},-1);
end
...
for scaleIdx = 1 : numel(scales)
    ...
    EBDFramePath = [DecFramePath,'QP',num2str(qScale),'/'];
    if ~exist(EBDFramePath, 'dir')
        mkdir(EBDFramePath);
    else
        rmdir(EBDFramePath,"s");
        mkdir(EBDFramePath);
    end
    ...
    Frame1stYUV = ictRGB2YCbCr(EBDFrame{1});
    zeroRunLenaSmall = IntraEncode(LenaSmallYUV, qScale);
    zeroRunFirstFrame = IntraEncode(Frame1stYUV, qScale);
    recFrame{1} = IntraDecode(zeroRunFirstFrame, size(OriFrame{1}), qScale);  % YCbCr
    %% Part 1: use pmf of zeroRunLenaSmall to build and train huffman table
    pmfFirstFrame = hist(zeroRunFirstFrame(:),min(zeroRunFirstFrame):max(zeroRunFirstFrame));  % based on the actual range of encoded image
    pmfFirstFrame = pmfFirstFrame./sum(pmfFirstFrame);  % use marg. PMF for 1D
    % Huffman Table
    ...
    % calculate bit rate
    rate(1) = (numel(bytestreamZeroRun)*8) / (numel(OriFrame{1})/3);
    ...
    FinalRecFrame{1} = IntraDecode(decZeroRun, size(OriFrame{1}), qScale);  % YCbCr
    FinalRecFrameRGB{1} = ictYCbCr2RGB(FinalRecFrame{1});
    imwrite(uint8(FinalRecFrameRGB{1}),[EBDFramePath,'Frame_1.bmp']);
    PSNR(1) = calcPSNR(OriFrame{1}, FinalRecFrameRGB{1});
    ...
    for i = 2 : videoLength
        ...
        imwrite(uint8(FinalRecFrameRGB{i}),[EBDFramePath,'Frame_',num2str(i),'.bmp']);
        rate(i) = rateZeroRun + rateMV;
        PSNR(i) = calcPSNR(OriFrame{i}, FinalRecFrameRGB{i});
        
    end
    %% Final: evaluation on the whole sequence: average PSNR and Bitrate
    bitrate_foreman(scaleIdx) = mean(rate);
    psnr_foreman(scaleIdx) = mean(PSNR);
    fprintf('QP: %.1f bit-rate: %.2f bits/pixel PSNR: %.2fdB\n', ...
        qScale, bitrate_foreman(scaleIdx), psnr_foreman(scaleIdx))
end
