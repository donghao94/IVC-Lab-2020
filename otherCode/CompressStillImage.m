...
DecForemanStillPath = '/Users/donghao/Desktop/DecForeman_StillImg/';
...
Frame       = cell(videoLength, 1);
OriFrame    = cell(videoLength, 1);
for i = 1:videoLength
    OriFrame{i}= double(imread([videoFolder, videoPath(i).name]));
    Frame{i}= bitshift(OriFrame{i},-1);
end
OriYUV = cellfun(@ictRGB2YCbCr, Frame, 'Uniform', false);
...
for scaleIdx = 1 : numel(scales)
    ...
    EBDFramePath = [DecForemanStillPath,'QP',num2str(qScale),'/'];
    mkdir(EBDFramePath);
    bpp = 0;
    psnr = 0;
    LenaSmall  = IntraEncode(LenaSmallYUV, qScale);
    %% use pmf of lena_small to build and train huffman table
    range = -1000:4000;  % based on the actual range of encoded image
    ...
    for i = 1 : videoLength
        ...
        ForemanYUV = OriYUV{i};
        k          = IntraEncode(ForemanYUV, qScale);
        %% use trained table to encode k to get the bytestream
        % encoding
        LowerBand = -1000;  % based on the actual range of encoded image
        bytestream = enc_huffman_new(k-LowerBand+1, BinCode, Codelengths);
        
        % calculate bit rate
        bpp = bpp + (numel(bytestream)*8) / (numel(Frame{i})./3);
        
        % decoding
        k_rec = double(reshape(dec_huffman_new(bytestream, BinaryTree,...
            max(size(k(:)))), size(k))) - 1 + LowerBand;
        %% image reconstruction
        I_rec = IntraDecode(k_rec, size(Frame{i}), qScale);
        I_rec = ictYCbCr2RGB(I_rec);
        imwrite(uint8(I_rec),[EBDFramePath,'Frame_',num2str(i),'.bmp']);
    ...
    fprintf('QP: %.1f bit-rate: %.2f bits/pixel PSNR: %.2fdB\n', ...
        qScale, BPPStill(scaleIdx), PSNRStill(scaleIdx))
end
