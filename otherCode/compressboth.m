...
OriFrameYUV = cellfun(@ictRGB2YCbCr, OriFrame, 'Uniform', false);
LenaSmall = double(imread('lena_small.tif'));
...
BPPStill  = zeros(1,11);
BPPVideo  = zeros(1,11);
PSNRStill = zeros(1,11);
PSNRVideo = zeros(1,11);
range = -1000:4000;
for scaleIdx = 1 : numel(scales)
    ...
    for i = 1 : videoLength
        ...
        % calculate bit rate
        bpp = bpp + (numel(bytestream)*8) / (numel(OriFrame{i})./3);
        % decoding
        k_rec = double(reshape(dec_huffman_new(bytestream, BinaryTree,...
            max(size(k(:)))), size(k))) - 1 + LowerBand;
        % image reconstruction
        I_rec = IntraDecode(k_rec, size(OriFrame{i}), qScale1);
        I_rec = ictYCbCr2RGB(I_rec);
        % calculate the psnr
        psnr = psnr + calcPSNR(OriFrame{i},I_rec);
%         imwrite(uint8(I_rec),[EBDStillFramePath,'Frame_',num2str(i),'.bmp']);
        
    end
    BPPStill(scaleIdx) = bpp ./ videoLength;
    PSNRStill(scaleIdx) = psnr ./ videoLength;
    fprintf('Still QP: %.1f bit-rate: %.2f bits/pixel PSNR: %.2f dB \n', ...
        qScale1, BPPStill(scaleIdx),PSNRStill(scaleIdx))
end
...
foreman= plot(bitrate_foreman, psnr_foreman, '-*', 'Color', 'r');
hold on
foreman_still= plot(bitrate_foremanstill, psnr_foremanstill, '-o', 'Color', 'r');
hold on
...
% foreman_opt = plot(bpp_1,psnr_1,'-*', 'Color', 'g');
...
% foreman_still_opt = plot(bpp_2,psnr_2,'-o', 'Color', 'g');
...
ylabel('PSNR [dB]');
legend([foreman,foreman_still], {'Foreman','ForemanOpt','Foreman-Still','Foreman-StillOpt'});
title('R-D Plot');
